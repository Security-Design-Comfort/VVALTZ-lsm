...
void print_usage()
{
  ...
  std::cout << "  --hide                  : Re-engage cloaking to hide the module\n";
  std::cout << "  --restore               : Uncloak kernel module to allow unloading\n";
...
int main(int argc, char *argv[])
{
  ...
  if (command == "--status") {
    if (print_vault_status() == false) {
      std::cerr << "Runtime Error: Proc connection dropped. Verify module is "
                   "loaded.\n";
      return 1;
    }
  }
    ...
    if (add_directory_to_vault(argv[2]) == false) {
      std::cerr << "Configuration Error: Failed writing target node path into "
                   "engine.\n";
      return 1;
    }
    ...
    if (remove_directory_from_vault(argv[2]) == false) {
      std::cerr << "Configuration Error: Failed purging target node path out "
                   "of engine.\n";
      return 1;
    }
    ...
    if (hide_module_visibility() == false) {
      std::cerr
          << "Runtime Error: Unable to communicate with driver interface.\n";
      return 1;
    }
    std::cout << "[SUCCESS] Dispatched cloaking request. Module is now hidden "
                 "from lsmod.\n";
  }

  else if (command == "--restore") {
    if (restore_module_visibility() == false) {
      std::cerr
          << "Runtime Error: Unable to communicate with driver interface.\n";
      return 1;
    }
    std::cout << "[SUCCESS] Dispatched uncloak request. Module is now visible "
                 "to rmmod.\n";
  ...
  //PRESET MANAGEMENT
    ...
    for (i = 0; i < SYSTEM_PRESETS.size(); i = i + 1) {
      if (SYSTEM_PRESETS[i].name == selected_preset) {
        preset_found = true;
        std::cout << "Parsing recommended configuration template: ["
                  << selected_preset << "]\n";
        ...
        for (j = 0; j < SYSTEM_PRESETS[i].paths.size(); j = j + 1) {
          ...
          if (add_directory_to_vault(absolute_target) == true) {
            std::cout << "  -> Secure protection armed for path: "
                      << absolute_target << "\n";
          } else {
            std::cerr << "  [!] Storage stream write error for path: "
                      << absolute_target << "\n";
          }
        }
      }
    ...
    if (preset_found == false) {
      std::cerr << "Deployment Error: Template named '" << selected_preset
                << "' does not exist. Run --preset-list to view entries.\n";
      return 1;
    }
  }
